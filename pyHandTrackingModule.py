...
class handDetector():
    def __init__(self, mode=False, maxHands=2, detectionCon=0.5, trackingCon=0.5):
        ...
        self.mp.Draw = mp.solutions.drawing_utils
            ...
            for lms in self.results.multi_hand_landmarks:
                if draw:
                    self.mpDraw.draw_landmarks(
                        img, lms, self.mpHands.HAND_CONNECTIONS)
        return img
        ...
        if self.results.multi_hand_landmarks:
            ...
            for id, lm in enumerate(myHand.landmarks):
                ...
                if draw:
                    ...
                    if draw:
                        cv2.circle(img, (bbox[0], bbox[1]),
                                   (bbox[2]+20, bbox[3]+20), (0, 255, 0))
    ...
    def findDistance(self, p1, p2, img, draw=True):
        ...
        x2, y2 = self.lmList[p2][2], self.lmList[p2][2]
        cx, cy = (x1 + x2) // 2, (y1 + y2) // 2
        if draw:
            cv2.circle(img, (x1, y1), 15, (255, 0, 255), cv2.FILLED)
            cv2.circle(img, (x2, y2), 15, (255, 0, 255), cv2.FILLED)
            cv2.line(img, (x1, y1), 15, (255, 0, 255), 3)
